...
const StatCard = ({ icon: Icon, target, label, suffix = '' }: { icon: React.ElementType, target: number, label: string, suffix?: string }) => {
  ...
  return (
    <div ref={ref} className="text-center">
      <Icon className="h-8 w-8 md:h-10 md:w-10 mx-auto mb-3 text-secondary" />
...
const GoldenVisa = () => {
  ...
  const testimonials = [
      { quote: "The Golden Visa process seemed daunting, but KAR's team made it incredibly simple and fast. Their expertise is unmatched. We received our visa in record time!", name: "Dr. Alistair Finch", title: "AI Researcher & Golden Visa Holder" },
      { quote: "As an investor, time is critical. KAR handled my entire application with utmost professionalism, allowing me to focus on my business. Highly recommended.", name: "Elena Petrova", title: "Real Estate Investor" }
  ];
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <section className="relative pt-40 pb-20 bg-gradient-hero text-white text-center">
        ...
        <div className="relative container mx-auto px-4">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
            ...
            <h1 className="text-4xl md:text-6xl font-heading font-bold mb-6">Unlock Your Future with the UAE Golden Visa</h1>
            <p className="text-lg md:text-xl text-white/90 max-w-3xl mx-auto mb-8">Secure a 10-year renewable residency with our expert-led, streamlined application process.</p>
              ...
              <motion.div key={selectedVisa.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.5 }} className="bg-card p-6 md:p-8 rounded-2xl border">
                <div className="grid lg:grid-cols-2 gap-8 md:gap-12">
                  <div>
                    ...
                    <h3 className="text-2xl md:text-3xl font-heading font-bold mb-4">{selectedVisa.name}</h3>
                  ...
                  <div className="bg-muted/50 p-6 rounded-lg flex flex-col">
                    <h3 className="text-xl md:text-2xl font-heading font-bold mb-6">Our Hassle-Free Process</h3>
      ...
      <section className="py-20 bg-muted/50">
          <div className="container mx-auto px-4">
              <div className="text-center mb-12 md:mb-16">
                  <h2 className="text-3xl md:text-5xl font-heading font-bold mb-6">Trusted by Global Achievers</h2>
                  <p className="text-lg md:text-xl text-muted-foreground max-w-3xl mx-auto">Hear from clients who successfully secured their future in the UAE with us.</p>
              </div>
              <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
                  {testimonials.map((testimonial, index) => (
                      <motion.div key={index} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: index * 0.1 }} className="bg-card p-8 rounded-2xl border">
